...
create table public.projects (
  ...
  description text,
  -- Location via the curated taxonomy (see locations migration), not
  -- free text -- this is what powers city/neighborhood browse pages.
...
-- Ownership/visibility helpers, mirroring owns_business / business_is_public.
...
comment on function public.owns_project(uuid) is
  'True if the calling user owns the business behind the given project.';
...
comment on function public.project_is_public(uuid) is
  'True if the given project is publicly visible (status = published).';
...
-- Projects may reference the specific services performed.
...
-- Projects may be tagged with one or more design styles.
...
-- Normalized project features (Water Feature, Fire Feature, ...) --
-- never hard-coded into project descriptions.
...
create table public.project_photos (
  id uuid primary key default gen_random_uuid(),
  project_id uuid not null references public.projects (id) on delete cascade,
  -- Supabase Storage object path. `url` may cache a public/signed URL, or
  -- (for seed/dev data only) point at an external placeholder image.
  storage_path text,
  url text,
  alt_text text,
  caption text,
  sort_order integer not null default 0,
  photo_type public.photo_type not null default 'gallery',
  created_at timestamptz not null default now(),
  constraint project_photos_source_present check (storage_path is not null or url is not null)
);
...
-- project_categories / project_services / project_styles / project_features / project_photos
-- all share the same visibility shape: public if the parent project is
-- published, owner-managed via the parent project's business, admin full access.
...
create policy "Owners manage their project categories"
...
create policy "Project services are publicly readable"
  on public.project_services for select
  to anon, authenticated
  using (public.project_is_public(project_id));

create policy "Owners manage their project services"
  on public.project_services for all
  to authenticated
  using (public.owns_project(project_id))
  with check (public.owns_project(project_id));

create policy "Admins manage project services"
  on public.project_services for all
  to authenticated
  using (public.is_admin())
  with check (public.is_admin());

create policy "Project styles are publicly readable"
  on public.project_styles for select
  to anon, authenticated
  using (public.project_is_public(project_id));

create policy "Owners manage their project styles"
  on public.project_styles for all
  to authenticated
  using (public.owns_project(project_id))
  with check (public.owns_project(project_id));

create policy "Admins manage project styles"
  on public.project_styles for all
  to authenticated
  using (public.is_admin())
  with check (public.is_admin());

create policy "Project features are publicly readable"
  on public.project_features for select
  to anon, authenticated
  using (public.project_is_public(project_id));

create policy "Owners manage their project features"
...
create policy "Owners manage their project photos"
