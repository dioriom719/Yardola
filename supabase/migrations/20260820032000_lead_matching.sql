...
-- active Yardola businesses using the V1 weighted scoring model.
...
-- SECURITY DEFINER avoids re-entering lead_matches RLS while the project_plans
-- policy is evaluating access for a matched business.
create or replace function public.business_can_view_plan(target_plan_id uuid)
returns boolean
...
comment on function public.business_can_view_plan(uuid) is
  'True when the calling business owns an active routed match for the plan.';

create policy "Matched businesses view their routed project plans"
  on public.project_plans for select
  to authenticated
  using (public.business_can_view_plan(id));
...
as $$
  ...
  quality as (
    select case when b.verification_status = 'verified' then 5 else 0 end
...
begin
  ...
  select email, first_name, last_name, phone into profile_email, profile_first_name, profile_last_name, profile_phone from public.profiles where id = caller_id;

  select l.id into new_lead_id from public.leads l where l.project_plan_id = target_plan_id and l.homeowner_id = caller_id order by l.created_at desc limit 1;
  if new_lead_id is null then
    ...
    values (target_plan_id, caller_id, nullif(trim(coalesce(profile_first_name, '') || ' ' || coalesce(profile_last_name, '')), ''), profile_email, profile_phone, 'new', 'planner')
    returning id into new_lead_id;
    insert into public.lead_events (lead_id, actor_id, event_type) values (new_lead_id, caller_id, 'created');
  else
    update public.leads set contact_name = nullif(trim(coalesce(profile_first_name, '') || ' ' || coalesce(profile_last_name, '')), ''), contact_email = profile_email, contact_phone = profile_phone, updated_at = now() where id = new_lead_id;
  end if;
  ...
  if inserted_count > 0 then
    insert into public.lead_events (lead_id, actor_id, event_type, metadata) values (new_lead_id, caller_id, 'matched', jsonb_build_object('match_count', inserted_count));
  end if;
  return query select new_lead_id, inserted_count;
end;
$$;

comment on function public.submit_project_plan(uuid) is 'Submit an owned homeowner plan, create/reuse its lead, and route it to the top matching active businesses.';
