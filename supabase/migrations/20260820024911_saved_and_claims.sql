-- Homeowner saved projects, and the business claim workflow.
...
comment on table public.saved_projects is 'Projects a homeowner has bookmarked.';
...
-- A business_user's request to take ownership of an existing (unclaimed)
-- business listing. Approval is an admin action (Phase 2: schema only --
-- the approval workflow that sets businesses.owner_id is a later phase).
...
comment on table public.business_claims is 'A claimant''s request to take ownership of a business listing.';
...
create policy "Homeowners manage their own saved projects"
  on public.saved_projects for all
  to authenticated
  using (user_id = (select auth.uid()))
  with check (user_id = (select auth.uid()));

create policy "Admins manage saved projects"
  on public.saved_projects for all
  to authenticated
