-- Phase 8: lead matching and routing.
--
-- Turns the Phase 2 lead data shape into a deterministic V1 matcher.
-- Score weights intentionally mirror the product spec:
--   category       30%
--   location       25%
--   service        20%
--   project type   15%
--   business quality 10%
--
-- V1 does not invent a service selection that the planner does not collect.
-- Instead, service fit is derived from whether the business offers at least
-- one service belonging to a category selected by the homeowner. Project
-- type is the plan's primary category (project_plans.category_id).
...
begin
  ...
  inserted as (
    ...
    do update set match_score = excluded.match_score,
                  ...
                  updated_at = now()
    returning business_id, match_score
  )
  select i.business_id, i.match_score
  from inserted i;
...
-- A small convenience wrapper for the planner submission flow. It creates
-- the private lead, records the creation event, then immediately matches it.
...
begin
  ...
  if not exists (
    ...
    where id = plan_id and user_id = v_uid
  ) then
    raise exception 'Project plan not found';
  end if;
  ...
  values (
    ...
    v_uid,
    'created',
    jsonb_build_object('source', 'planner')
  );
...
comment on function public.submit_project_lead(uuid, text, text, text) is
  'Create a planner lead for the authenticated homeowner and immediately route it through the V1 matcher.';

-- Keep the public API surface narrow: these functions are callable by
-- authenticated users only. SECURITY DEFINER performs the cross-table
-- inserts while the functions themselves enforce ownership.
...
revoke all on function public.submit_project_lead(uuid, text, text, text) from public;
grant execute on function public.submit_project_lead(uuid, text, text, text) to authenticated;
