...
export async function submitBusinessClaim(
  ...
  const verificationInfo = String(
    formData.get("verificationInfo") ?? ""
  ).trim();
  if (!verificationInfo)
    throw new Error("Tell us why you represent this business.");
  const { supabase, user } = await requireUser();
  ...
  const { error } = await supabase.from("business_claims").insert({
    ...
    claimant_id: user.id,
    verification_info: { note: verificationInfo },
  });
  if (error) {
    if (error.code === "23505")
...
export async function updateBusinessProfile(
  ...
  if (error) throw error;
  revalidatePath("/business");
  revalidatePath("/professionals/[slug]", "page");
}

export async function updateBusinessTaxonomy(
  businessId: string,
  formData: FormData
) {
  const { supabase } = await requireUser();
  ...
  if (cityIds.length) {
    const { error } = await supabase
      .from("business_service_areas")
      .insert(cityIds.map((city_id) => ({ business_id: businessId, city_id })));
    if (error) throw error;
  }
  revalidatePath("/business");
  revalidatePath("/professionals/[slug]", "page");
